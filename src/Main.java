...
public class Main {
    ...
    private static HashMap<Character, Boolean> hasLambda = new HashMap<>();
    private static boolean acepted = false;
    ...
    public static void main(String[] args) throws IOException{
        ...
        try{
            ...
            file = new File("C:/Users/Navarro D. Gerardo/Documents/Quinto Semestre/mate computacional/Part2/src/test3.txt");
        ...
        while(scan.hasNext()){ //while we have lines to read we will continue reading the file
            ...
            if(content.equals("lmd")){
                hasLambda.put(key, true);
            }else{
                if(!hasLambda.containsKey(key)){
                    hasLambda.put(key, false);
                }
            }
        ...
        System.out.println("Production of the grammar: " + hasLambda);
        ...
        PDA.add(start);
        String testString = "bbb";//tenemos que cambiar esta linea por un scanner para que usuario lo ponga
        System.out.println("string to test: " + testString);
    ...
    private static void checkString(Stack<Character> PDA, String testString){
        if(!acepted) {
            //System.out.println(PDA + " -> " + testString);
            if(PDA.size() == 1 && testString.equals("")){
                if(hasLambda.containsKey(PDA.peek()) && hasLambda.get(PDA.peek())){
                    acepted = true;
                }
            }
            ...
            if(!PDA.empty()){
                ...
                if (transitions.containsKey(c)) {
                    int counterLambda = 0;
                    if(hasLambda.containsKey(c) && hasLambda.get(c)){
                        counterLambda++;
                    }
                    ...
                    if(sb.length() + 1 > testString.length() + counterLambda){
                        return;
                    }
